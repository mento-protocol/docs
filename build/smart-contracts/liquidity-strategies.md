...
6. Strategy’s **onRebalance** validates that the caller is the pool and that the sender is the strategy itself (transient storage / reentrancy guard). Then it calls an internal **\_handleCallback** (implemented by Reserve vs CDP):
