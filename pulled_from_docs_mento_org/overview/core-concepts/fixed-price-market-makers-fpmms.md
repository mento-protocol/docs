...
## Inventory Rebalancing (v3)

One-sided trading creates inventory imbalances. Mento v3 handles this through **strategy-mediated rebalancing**:

**Drift detection**: Pools monitor the ratio between reserves and the oracle price. When the "reserve price" drifts beyond configured **thresholds** (separate above/below), rebalancing becomes *eligible*.

**Allowlisted strategies only**: Only **allowlisted liquidity strategies** (Reserve, CDP, or third-party) may call the pool’s rebalance function. The pool sends one token out and calls back into the strategy; the strategy returns the other token at the oracle rate.

**Target is the boundary, not 50/50**: Strategies rebalance toward a **threshold boundary** (e.g. the band implied by the threshold), not to exact 50/50. This caps rebalance size relative to the trigger trade and limits attack surface.

**Keeper incentives**: Anyone can trigger a strategy’s `rebalance(pool)` (permissionless); the strategy enforces cooldown and then calls the pool. Rebalance incentive is capped (pool enforces minimum repayment).

**Safety checks**: Rebalancing must improve deviation, preserve direction (above oracle stays above, etc.), not overshoot past the band, and satisfy a minimum repayment (incentive cap). Value protection and trading limits apply as on swaps.
...
* [Stability Mechanisms](https://docs.mento.org/mento/overview/core-concepts/stability-mechanisms) - How fixed prices maintain pegs
* [The Reserve](https://docs.mento.org/mento/overview/core-concepts/the-reserve) - Collateral backing for exchanges
* [Oracles & Price Feeds](https://docs.mento.org/mento/overview/core-concepts/oracles-and-price-feeds) - Price discovery for FPMMs
* [Trading Limits & Circuit Breakers](https://docs.mento.org/mento/overview/core-concepts/trading-limits-and-circuit-breakers) - Safety mechanisms for pools
